...
class ALExplorer(Explorer):
    ...
    :meth:`is_done`. By default this counter is never auto-reset; call
    :meth:`reset_episode_tracking` before another episode when reusing the
    same explorer, or set ``auto_reset_episode_tracking=True``.
    ...
    Args:
        ...
        auto_reset_episode_tracking (bool): Whether :meth:`run_episode`
            should call :meth:`reset_episode_tracking` before every rollout.
    ...
    def __init__(
        ...
        auto_reset_episode_tracking: bool = False,
    ):
        ...
        self.auto_reset_episode_tracking = bool(auto_reset_episode_tracking)
    ...
    def reset_episode_tracking(self) -> None:
        """Reset episode metrics while retaining the learner and callbacks.

        Call this before another :meth:`run_episode` on the same explorer.
        It restores the sampling budget and clears the best value reported for
        the previous episode. State owned by the active-learning callbacks,
        such as accumulated observations or trained models, is not modified.
        """
        self.samples_collected = 0
        self._exhausted_candidates = False
        self.best_value_observed = None
        self.best_position_observed = None

    def run_episode(
        self,
        env=None,
        render: bool = False,
        reset_episode_tracking: Optional[bool] = None,
    ) -> dict:
        """Run one episode, optionally clearing AL per-episode metrics first.

        Args:
            env: Optional environment override. Defaults to ``self.env``.
            render (bool): Forwarded to :meth:`agent.Agent.run_episode`.
            reset_episode_tracking (Optional[bool]): Per-call override for
                :attr:`auto_reset_episode_tracking`. When ``None``, the
                instance-level flag is used.

        Returns:
            dict: Episode summary from :meth:`agent.Agent.run_episode`.
        """
        should_reset = (
            self.auto_reset_episode_tracking
            if reset_episode_tracking is None
            else bool(reset_episode_tracking)
        )
        if should_reset:
            self.reset_episode_tracking()
        return super().run_episode(env=env, render=render)

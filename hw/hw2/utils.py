...
class Position(NamedTuple):
    """Integer 2D position ``(x, y)`` on the map grid.

    Using a named tuple keeps positions hashable (so they can serve as keys in
    dictionaries that track sampled cells or vehicle locations) while still
    behaving like a 2-tuple for unpacking and equality checks.
    """
...
def validate_depth_map(depth_map: Optional[np.ndarray]) -> Optional[np.ndarray]:
    """Return a validated depth-map matrix or ``None``."""
    if depth_map is None:
        return None

    validated_depth_map = np.asarray(depth_map, dtype=float)
    if validated_depth_map.ndim != 2:
        raise ValueError("depth_map must be a 2D matrix")
    if min(validated_depth_map.shape) < 2:
        raise ValueError("depth_map must have at least 2 rows and 2 columns")
    return validated_depth_map
...
def get_random_grid_position(env) -> Position:
    """Get a random sampling-grid position."""
    position = Position(
        int(np.random.choice(env.x_grid_coords)),
        int(np.random.choice(env.y_grid_coords)),
    )
    return position
